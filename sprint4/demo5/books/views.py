...
class BookMarkView(APIView):
    ...
    def get(self, request: Request, book_id: int) -> Response:
        ...
        self.check_object_permissions(request, book_obj)
    ...
    def post(self, request: Request, book_id: int) -> Response:
        """
        1. Objeto de Book (book_id)
        2. Objeto de Account (request.user)
        3. Informações da marcação (chapter, note) (body do post -> request.data)
        """
        book_obj = get_object_or_404(Book, pk=book_id)
        self.check_object_permissions(request, book_obj)
        ...
        serializer.is_valid(raise_exception=True)
        serializer.save(book=book_obj, marker=request.user)
